...
class PgsqlConnector():
    ...
    def data_fetch(self):  
        ...
        # idcs_rows = json.dumps(idcs_rows, indent=2, default=str)
        # print(idcs_rows)
        ...
        for ir in idcs_rows:
            for er in ecs_rows:
                self.idc_to_ec.append({ir['id']: (ir["lng"], ir["lat"]) ,
                                        er['id']: (er["lng"], er["lat"])})
        for dr in das_rows:
            # for er in ecs_rows:
            #     self.da_to_ec.append({dr['id']: (dr["lng"], dr["lat"]) ,
            #                             er['id']: (er["lng"], er["lat"])})
            for tr in tmcs_rows:
                self.da_to_tmc.append({dr['id']: (dr["lng"], dr["lat"]) ,
                                        tr['id']: (tr["lng"], tr["lat"])})
            # for hr in hospitals_rows:
            #     self.da_to_h.append({dr['id']: (dr["lng"], dr["lat"]) ,
            #                             hr['id']: (hr["lng"], hr["lat"])})
            if cur:
                cur.close()
            if conn:
