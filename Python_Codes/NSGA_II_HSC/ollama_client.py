...
class OllamaClient:
    """
    کلاینت برای ارتباط با Ollama محلی و دریافت پیشنهادهای هوشمند برای NSGA-II.
    ...
    def __init__(
        ...
        """
        Constructor برای Ollama Client
        ...
        # برای حفظ سازگاری با اینترفیس قبلی
...
"""
    ...
    def send_request(
        self, prompt: Dict[str, str], max_retries: int = 3
    ) -> Optional[Dict[str, Any]]:
        """
        ارسال درخواست ساده به Ollama (بدون state)
        """
        payload = {
            "model": self.model,
            "messages": [
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    formatted = self._format_ollama_response(response.json())
                    if formatted:
                        return formatted
                    print(
                        f"⚠️ Empty or invalid response from Ollama (attempt {attempt + 1})"
                    )
                else:
                    print(
...
From now on, I will only send you the history of the parameters without any additional writing, and you will return your suggestions by analyzing them carefully, completely in accordance with the output format."""
    ...
    def load_previous_response_id(self) -> Optional[str]:
        ...
        """
        if not self.previous_response_id_file:
            return None
        try:
    ...
    def send_request_openai(
        ...
        store: bool = True,  # سازگاری با اینترفیس قبلی
        max_retries: int = 3,
    ) -> Optional[Dict[str, Any]]:
        """
        ارسال درخواست به API محلی Ollama (بدون استریم)
        """
    ...
    def _format_ollama_response(self, data: Any) -> Optional[Dict[str, Any]]:
        """
        تبدیل پاسخ Ollama به ساختار مشابه OpenAI (choices -> message -> content)
        """
