    ...
    def main(self):
        ...
        if len(pf_costs) > 0:
            ...
            if pf_costs.shape[1] == 3:
                ...
                # ایجاد ماتریس trade-off
                tradeoff_matrix = np.zeros((3, 3))
                tradeoff_matrix[0, 1] = metrics.get('F1_F2', {}).get('mean_tradeoff', 0)
                tradeoff_matrix[0, 2] = metrics.get('F1_F3', {}).get('mean_tradeoff', 0)
                tradeoff_matrix[1, 2] = metrics.get('F2_F3', {}).get('mean_tradeoff', 0)
