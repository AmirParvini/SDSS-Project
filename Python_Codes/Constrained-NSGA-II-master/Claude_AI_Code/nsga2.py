...
# @author: YanSen
# @date: 2023/05/03
# Modified to support new Problem class structure
...
class ConstrainedNSGA2(object):
    ...
    Modified to work with the new Problem class structure
    """
    ...
    def run(self):
        ...
        for _ in tqdm(range(self._max_gen), desc="Generation Progress",
            ...
            selected_parent_pop = self._tournament_selection(population)
            # Create child population through genetic operators
    ...
    def _plot_pareto_front(self, population):
        ...
        if self._n_objs == 2:
            ...
            x = population[:, self._n_vars]
            y = population[:, self._n_vars + 1] 
            z = population[:, self._n_vars + 2]
            ax.scatter(x, y, z, s=50, c='blue', marker='.', edgecolors='darkgray', alpha=0.8, label='Pareto Front')
            ax.set_xlabel('Total Facilities', fontsize=12, labelpad=10)
            ax.set_ylabel('Weighted Distance', fontsize=12, labelpad=10)
            ax.set_zlabel('Unmet Demand Penalty', fontsize=12, labelpad=10)
            ax.view_init(elev=30, azim=35)
            plt.title('TDR Facility Allocation - Pareto Front', fontsize=16)
        else:
    ...
    def _evaluate_population(self, pop_vars):
        ...
        for ind_idx in range(self._pop_size):
            # Get objectives and constraint violations from problem
            objs, violations = self._problem.get_individual_result(pop_vars[ind_idx, :])
            pop_objs[ind_idx, :] = objs
            
            # Handle constraint violations
            if violations.size > 0:
                pop_violation[ind_idx, :min(self._n_cons, len(violations))] = violations[:min(self._n_cons, len(violations))]
    ...
    def __normalize_constraint_violation(pop_violation):
        """
        Normalize the constraint violations of various individuals.
        """
        if pop_violation.size == 0:
            return np.zeros(pop_violation.shape[0])
            
        violation_max = pop_violation.max(axis=0)
        normalized_violations = np.divide(pop_violation, violation_max, out=np.zeros_like(pop_violation),
    ...
    def _tournament_selection(self, original_pop):
        """
        Parents are selected from the population pool for reproduction using binary tournament selection.
        """
    ...
    def __simulated_binary_crossover(self, parent1, parent2):
        ...
        if np.random.random() <= self._pc:
            ...
            for var_idx in range(self._n_vars):
                ...
                ) / (abs(parent2_var_value - parent1_var_value) + 1e-10)
    ...
    def __polynomial_mutation(self, original_individual):
        """
        Perform polynomial mutation on a given individual.
        """
        # Handle division by zero
        range_vars = self._vars_ub - self._vars_lb
        range_vars[range_vars == 0] = 1.0
        
        norm_ind = np.minimum(original_individual - self._vars_lb,
                              self._vars_ub - original_individual) / range_vars
        ...
        result_ind = original_individual + delta_value * mutated_loc * range_vars

        # Ensure bounds
        result_ind = np.clip(result_ind, self._vars_lb, self._vars_ub)
    ...
    def _non_dominated_sorting_and_crowding_distance(self, population: np.ndarray) -> tuple:
        ...
        if problem_type <= 0.5 and feasible_pop is not None:
        ...
        if problem_type >= 0.5 and infeasible_pop is not None:
